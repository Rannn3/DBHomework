...
class LRUKReplacer {
 ...
 private:
  ...
  struct LRUKNode {
    ...
    std::list<size_t> history_;  // List of access timestamps, newest at front
  };
  ...
  std::list<frame_id_t> history_list_;  // Frames with < k accesses (LRU order)
  std::list<frame_id_t> cache_list_;    // Frames with >= k accesses
